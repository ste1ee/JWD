...
public class Primer2TryCatchFinally {
	...
	public static void main(String[] args) {
		do {	
			...
			BufferedReader in = null; // deklaracija mora biti van try statement-a ako se koristi u finally bloku
			try {
				in = new BufferedReader(new FileReader(ISPRAVNA)); // ova linija može da proizvede Exception
				...
				// nekad i naredbe u finally bloku mogu da proizvedu izuzetke
				try { in.close(); } catch (Exception ex) {
					// swallowed exception (u ovom slučaju svakako više ništa ni ne može da se uradi)
				}
			}
